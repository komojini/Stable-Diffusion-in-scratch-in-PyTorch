...
class UNET_AttentionBlock(nn.Module):
    def __init__(self, n_heads: int, embed_dim: int, d_context=768):
        super().__init__()
        channels = embed_dim // n_heads

        self.groupnorm = nn.GroupNorm(32, channels, eps=1e-6)
        self.conv_input = nn.Conv2d(channels, channels, kernel_size=1, padding=0)

        self.layernorm_1 = nn.LayerNorm(channels)
        self.self_attn = SelfAttention(n_heads, channels, in_proj_bias=False)
        self.layernorm_2 = nn.LayerNorm(channels)
        self.cross_attn = CrossAttention(n_heads, channels, d_context, in_proj_bias=False)
    ...
    def forward(self, x, context):
        ...
        # Normalizatoin + Cross Attention with skip connection
        ...
        # Normalizatoin + Feed Forward with skip connection
...
class UNET_ResidualBlock(nn.Module):
    ...
    def forward(self, feature: torch.Tensor, time: torch.Tensor) -> torch.Tensor:
        """
        Args:
            feature: (batch_size, in_channels, latent_h, latent_w) 
            time: (1, 1280)
        """
        residue = feature
        ...
        return self.residual_layer(residue) + merged
